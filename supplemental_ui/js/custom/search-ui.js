(function (global, factory) {
  ...
  function searchIndex (index, store, text) {
    ...
    if (result.length > 0) {
      result.sort(function(a,b) {
        const a_doc = getVersion(a, store)
        const b_doc = getVersion(b, store)
        if (a_doc !== b_doc) {return ('' + b_doc).localeCompare(a_doc);}
  ...
  function getVersion(element, store){
    const ids = element.ref.split('-');
    const docId = ids[0];
    const doc = store.documents[docId];
    return doc.version;
  }
